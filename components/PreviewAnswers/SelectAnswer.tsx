...
export const SelectAnswer: React.FunctionComponent<Props> = props => {
  const { answer } = props;
  ...
  return (
    <QuestionFeedback skipped={answer.value === "skip"}>
      ...
      <Grade value={answer.question.options.indexOf(answer.value) + 1} max={answer.question.options.length} />
    </QuestionFeedback>
  );
};
